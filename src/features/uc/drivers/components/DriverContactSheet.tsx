...
const DISABLED_TINT_BG = '#F1F5F9';
const DISABLED_TINT_FG = '#94A3B8';
...
export const DriverContactSheet = forwardRef<BottomSheetModal, Props>(
  ({ driver, onDismiss }, ref) => {
    ...
    const whatsappNumber = useCallback(
      async (phone: string) => {
        ...
        const r = await openWhatsApp(
          phone,
          `Hi ${driver.name.split(' ')[0]}, this is Urban Cruise.`,
        );
        if (!r.ok)
    ...
    const handleCall = useCallback(() => {
      if (!driver) return;
      if (!driver.phoneAlt || driver.phone === driver.phoneAlt) {
        dialNumber(driver.phone);
        return;
      }
      setPickerMode('call');
    }, [driver, dialNumber]);
    ...
    const handleEmail = useCallback(async () => {
      ...
      const r = await sendEmail({
        to: driver.email,
        subject: 'Urban Cruise — Driver follow up',
        body: `Hi ${driver.name.split(' ')[0]},\n\n`,
      });
    ...
    return (
      ...
      >
        {pickerMode && driver && driver.phoneAlt ? (
          <BottomSheetView style={styles.pickerBody}>
            ...
            <PickerRow
              label="Primary"
              value={driver.phone}
              onPress={() => handlePickerPick(driver.phone)}
            />
            <PickerRow
              label="Alternate"
              value={driver.phoneAlt}
              onPress={() => handlePickerPick(driver.phoneAlt as string)}
            />
            <Pressable
...
DriverContactSheet.displayName = 'DriverContactSheet';
...
const DriverSheetContent: React.FC<ContentProps> = ({
  driver,
  onCall,
  onWhatsApp,
  onEmail,
  ...
  return (
    <View>
      ...
      <View style={styles.quickRow}>
        ...
        <QuickAction
          label="Email"
          onPress={hasEmail ? onEmail : undefined}
      ...
      <View style={styles.infoBlock}>
        <InfoRow Icon={Phone} label="Primary phone" value={driver.phone} />
        {driver.phoneAlt && (
          <InfoRow
            Icon={Phone}
        ...
        <InfoRow Icon={Building2} label="Vendor" value={driver.vendorName} />
        <InfoRow
          Icon={CreditCard}
          label="License no."
...
const QuickAction: React.FC<QAProps> = ({
  label,
  onPress,
  bg,
  fg,
